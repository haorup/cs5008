...
void bubblesort(int* array, unsigned int size) {
    // Build a loop invariant to start a iteration.
    // Loop over every elements in the array.
    for (int i = 0; i < size - 1; i++) {
        // Build a loop invariant to start a iteration 
        // Compare the neighbored elements in each iteration
        // and switch them if they are not ordered.
        for (int j = 0; j < size - 1; j++) {
            // Compare each element with the next one.
            // if the next one is a bigger number, swap them
            // if not, remain the same position.
